...
const refreshRoles = async () => {
  ...
  list.innerHTML = roles.map(r => `<li>${r.id} — ${r.name}</li>`).join('') || '<li>(none)</li>';
};
...
const refreshUsers = async () => {
  ...
  list.innerHTML = users.map(u => `<li data-id="${u.id}">${u.id} — ${u.username} — roles: [${u.roles.join(',')}]</li>`).join('') || '<li>(none)</li>';
};
