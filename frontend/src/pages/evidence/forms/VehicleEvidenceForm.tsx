...
export default function VehicleEvidenceForm() {
  ...
  const fetchEvidence = async () => {
    try {
      ...
      console.error('Failed to fetch vehicle evidence:', err);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'خطا در ثبت مدرک وسیله نقلیه');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="main-content">
        <div className="evidence-form-container">
          <form className="evidence-form" onSubmit={handleSubmit}>
            <h2>{id ? 'ویرایش مدرک وسیله نقلیه' : 'ثبت مدرک وسیله نقلیه'}</h2>
            ...
            <div className="form-group"><label>مدل *</label><input type="text" value={formData.model_name} onChange={(e) => setFormData({ ...formData, model_name: e.target.value })} required /></div>
            <div className="form-group"><label>رنگ *</label><input type="text" value={formData.color} onChange={(e) => setFormData({ ...formData, color: e.target.value })} required /></div>
            <div className="form-group"><label>پلاک</label><input type="text" value={formData.license_plate} onChange={(e) => setFormData({ ...formData, license_plate: e.target.value })} /></div>
            <div className="form-group"><label>شماره سریال</label><input type="text" value={formData.serial_number} onChange={(e) => setFormData({ ...formData, serial_number: e.target.value })} /></div>
