...
export default function CaseDetail() {
  ...
  const isGuiltyByPolice = (s: Suspect) => {
    if (!s.interrogations || s.interrogations.length === 0) return false;
    const latestInterrogation = s.interrogations[s.interrogations.length - 1];
    const feedback = latestInterrogation.feedback;
    if (!feedback) return false;
    
    // Level 0 (Critical) needs Chief confirmation
    if (caseData?.crime_level === 0) {
      return feedback.is_chief_confirmed === true && feedback.decision === 'GUILTY';
    }
    
    // Others need Captain confirmation
    return feedback.is_confirmed === true && feedback.decision === 'GUILTY';
  };
          ...
          <div className="case-info-grid">
            ...
            {caseData.scene_data && (
              <div className="info-section">
                ...
                <p><strong>زمان وقوع:</strong> {new Date(caseData.scene_data.occurrence_time).toLocaleString('fa-IR', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</p>
            ...
            <div className="review-section">
              ...
              {verdicts.map((v) => (
                <div key={v.id} className="lux-card" style={{ marginBottom: '16px', border: '1px solid rgba(212, 175, 55, 0.2)' }}>
                  ...
                  <p><strong>متهم:</strong> {suspects.find(s => s.id === v.suspect)?.first_name} {suspects.find(s => s.id === v.suspect)?.last_name}</p>
                  ...
                  {/* Bail and Fine Management - Show for all GUILTY verdicts */}
                  {v.result === 'GUILTY' && (
                    <div style={{ marginTop: '15px', padding: '12px', background: 'rgba(212, 175, 55, 0.05)', borderRadius: '8px', border: '1px solid rgba(212, 175, 55, 0.2)' }}>
                      <h5 style={{ color: 'var(--primary)', marginBottom: '8px', fontSize: '0.9rem' }}>💰 وثیقه و جریمه</h5>
                      
                      {v.is_eligible_for_bail !== undefined && (
                        <p style={{ fontSize: '0.85rem', marginBottom: '8px' }}>
                          <strong>واجد شرایط وثیقه:</strong>{' '}
                          {v.is_eligible_for_bail ? (
                            <span style={{ color: '#10b981' }}>✓ بله (جرم سطح {v.case_crime_level})</span>
                          ) : (
                            <span style={{ color: '#ef4444' }}>✗ خیر (جرم سطح {v.case_crime_level})</span>
                          )}
                        </p>
                      )}
                      ...
                      {v.bail_amount && (
                        <p style={{ fontSize: '0.85rem', marginBottom: '4px' }}>
                          <strong>وثیقه:</strong> {v.bail_amount.toLocaleString('fa-IR')} تومان
                          {v.bail_paid ? (
                            <span style={{ color: '#10b981', marginRight: '8px' }}>✓ پرداخت شده</span>
                          ) : (
                            <span style={{ color: '#ef4444', marginRight: '8px' }}>✗ پرداخت نشده</span>
                          )}
                        </p>
                      )}
                      {v.fine_amount && (
                        <p style={{ fontSize: '0.85rem', marginBottom: '4px' }}>
                          <strong>جریمه:</strong> {v.fine_amount.toLocaleString('fa-IR')} تومان
                          {v.fine_paid ? (
                            <span style={{ color: '#10b981', marginRight: '8px' }}>✓ پرداخت شده</span>
                          ) : (
                            <span style={{ color: '#ef4444', marginRight: '8px' }}>✗ پرداخت نشده</span>
                      ...
                      {!v.bail_amount && !v.fine_amount && (
                        <p style={{ fontSize: '0.85rem', color: 'var(--text-dim)', marginBottom: '8px' }}>
                          مبالغ وثیقه و جریمه هنوز تعیین نشده‌اند.
                        </p>
                      )}
                      
                      {v.bail_tracking_code && <p style={{ fontSize: '0.75rem', color: 'var(--text-dim)' }}>کد رهگیری وثیقه: {v.bail_tracking_code}</p>}
                      {v.fine_tracking_code && <p style={{ fontSize: '0.75rem', color: 'var(--text-dim)' }}>کد رهگیری جریمه: {v.fine_tracking_code}</p>}
                      
                      <button 
                        className="btn-gold-outline" 
                        style={{ marginTop: '10px', padding: '6px 12px', fontSize: '0.8rem' }}
                        onClick={() => navigate(`/bail-payment/${v.id}`)}
                      >
                        مدیریت پرداخت
                      </button>
                    </div>
              ...
              {canJudgeVerdict && (
                <div className="mini-list">
                  <h4 className="gold-text" style={{ fontSize: '0.9rem', marginBottom: '10px' }}>لیست متهمین واجد شرایط محاکمه:</h4>
                  {suspects
                    .filter(s => isGuiltyByPolice(s) && !verdicts.some(v => v.suspect === s.id))
                    .length > 0 ? (
                      suspects
                        .filter(s => isGuiltyByPolice(s) && !verdicts.some(v => v.suspect === s.id))
                        .map(s => (
                          <div key={s.id} className="mini-list-item" style={{ justifyContent: 'space-between', background: 'rgba(212,175,55,0.05)', border: '1px solid rgba(212,175,55,0.1)' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                              <span>⚖️ {s.first_name} {s.last_name}</span>
                              <span style={{ background: '#92400e', color: '#fde68a', fontSize: '0.6rem', padding: '1px 4px', borderRadius: '3px' }}>آماده محاکمه</span>
                            </div>
                            <button 
                              className="btn-gold-solid" 
                              style={{ padding: '4px 12px', fontSize: '0.8rem' }}
                              onClick={() => navigate(`/cases/${caseData.id}/trial/${s.id}`)}
                            >
                              محاکمه
                            </button>
                          </div>
                      ...
                      <p className="no-data">متهمی با تشخیص گناهکار توسط پلیس یافت نشد یا همگی محاکمه شده‌اند.</p>
                    )
                  }
                </div>
              )}
