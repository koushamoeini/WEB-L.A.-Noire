...
const CreateCaseScene = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Append local timezone offset so Django stores the correct UTC time
      const pad = (n: number) => String(n).padStart(2, '0');
      const tzOffsetMin = -new Date().getTimezoneOffset();
      const tzSign = tzOffsetMin >= 0 ? '+' : '-';
      const tzAbs = Math.abs(tzOffsetMin);
      const tzSuffix = `${tzSign}${pad(Math.floor(tzAbs / 60))}:${pad(tzAbs % 60)}`;
      const isoDateTime = `${gDate.year}-${pad(gDate.month)}-${pad(gDate.day)}T${selectedTime.hour}:${selectedTime.minute}:00${tzSuffix}`;
